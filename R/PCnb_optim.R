...
#' This function estimates adjacency matrix of a NB models given a matrix of counts, using optim function.
#'
...
nbscale.noT <- function(X,maxcard,alpha, extend){
  ...
  iter.theta <- 2
  stop.epsilon <- .0001
  zeta <-  try(foreach(i = 1:p, .combine = "cbind",.export = c("nb.regression.parseModel",
