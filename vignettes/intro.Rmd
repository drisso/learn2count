...
plot(graph_est)
## est2 <- learn2count:::gp.lr(mat, maxcard=3, alpha=0.1, overdispersion=FALSE, extend=TRUE)
## colnames(est2) <- as.character(1:10)
## graph_est2 <- graph_from_adjacency_matrix(est2, mode="undirected")
```
...
We can now visualize the resulting graph, e.g., with the gRbase package.

```{r visualize}
graph_est <- graph_from_adjacency_matrix(metadata(top)$adj_mat, mode="undirected")
